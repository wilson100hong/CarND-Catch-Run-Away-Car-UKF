  ...
  h.onMessage([&ukf,&target_x,&target_y](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode) {
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2')
    {
      ...
      if (s != "") {
        ...
        // Adaptive target esitmation depends on current distance.
        double est_x = ukf.x_[0];
        double est_y = ukf.x_[1];
        double est_v = ukf.x_[2];
        double est_dist = std::hypot(est_x - hunter_x, est_y - hunter_y);
        double factor = std::min(2.0, est_dist / est_v);
        target_x = ukf.x_[0] + ukf.x_[2] * cos(ukf.x_[3]) * factor;
        target_y = ukf.x_[1] + ukf.x_[2] * sin(ukf.x_[3]) * factor;
